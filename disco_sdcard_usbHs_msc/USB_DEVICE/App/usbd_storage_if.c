...
unsigned long _LogBlockNbr,_LogBlockSize;
...
int8_t STORAGE_Init_HS(uint8_t lun)
{
  /* USER CODE BEGIN 9 */
	...
	if (HAL_SD_GetCardInfo(&hsd1, &pCardInfo)) {
		return (USBD_FAIL);
	}

	_LogBlockNbr = pCardInfo.LogBlockNbr - 1;
	_LogBlockSize = pCardInfo.LogBlockSize;
	printf("block_num %d\r\n",pCardInfo.LogBlockNbr - 1);
...
{
  /* USER CODE BEGIN 10 */
	*block_num =_LogBlockNbr;
	*block_size = _LogBlockSize;
	return (USBD_OK);
  /* USER CODE END 10 */
}
...
{
  /* USER CODE BEGIN 13 */
	//UART_Print("read adr %d len %d\r\n",blk_addr,blk_len);
	//if (HAL_SD_ReadBlocks_DMA(&hsd1, buf, blk_addr, blk_len)
	//printf("rd %d\r\n",blk_addr);

	if (HAL_SD_ReadBlocks(&hsd1, buf, blk_addr, MSC_MEDIA_PACKET/_LogBlockSize, 300)
	//if (HAL_SD_ReadBlocks(&hsd1, buf, blk_addr, blk_len, 300)
			!= HAL_OK) {
		return (USBD_FAIL);
	}
...
{
  /* USER CODE BEGIN 14 */
	/*printf("wr 0x%08X len%d\r\n", blk_addr*kt,blk_len);
    printf("Data: \"");
    printf("%.*s", kt, buf);  // kt uzunluğunda char dizisi olarak yazdır
    printf("\"\r\n");*/

	//if (SD_write(lun, buf, blk_addr, blk_len)
	if (HAL_SD_WriteBlocks( &hsd1,buf, blk_addr, MSC_MEDIA_PACKET/_LogBlockSize, 300)
			!= HAL_OK) {
		return (USBD_FAIL);
	}
	uint32_t timeStart = HAL_GetTick();
