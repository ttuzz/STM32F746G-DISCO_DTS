...
#define _BLK_NBR                  ((16*1024*1024)/MSC_MEDIA_PACKET)
#define _BLK_SIZ                  MSC_MEDIA_PACKET
...
int8_t STORAGE_Read_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  /* USER CODE BEGIN 13 */
	//printf("rd %d\n",blk_addr);
	BSP_QSPI_Read(buf,blk_addr*_BLK_SIZ,_BLK_SIZ);
  return (USBD_OK);
  /* USER CODE END 13 */
}
...
int8_t STORAGE_Write_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  /* USER CODE BEGIN 14 */
	//printf("wr %d\n",blk_addr);
	if((blk_addr*_BLK_SIZ%4096)==0){
		BSP_QSPI_Erase_SUBSECTOR(blk_addr*_BLK_SIZ/4096);
	}

  uint32_t addr = blk_addr * _BLK_SIZ;
  uint32_t size = blk_len * _BLK_SIZ;

  // QSPI Flash'a yazma
  if (BSP_QSPI_Write(buf, addr, size) != QSPI_OK) {
	return (USBD_FAIL);  // Hata kodu döndürülüyor
  }
